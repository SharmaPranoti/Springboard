...
# Extraxt the DataFrame excluding the 1st row
...
average_ratio_df = pd.DataFrame(average_ratio_data.items(), columns=['London_Boroughs','Average_Ratio']).sort_values(by='Average_Ratio',ascending=False)
...
plt.show()'''
final_bar_graph = average_ratio_df.head(10).plot(kind='bar', x='London_Boroughs', y='Average_Ratio')
final_bar_graph.set_ylabel('Average Price Ration')
plt.show()
